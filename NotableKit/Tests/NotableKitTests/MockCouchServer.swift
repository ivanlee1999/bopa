...
final class MockCouchServer: HTTPTransport, @unchecked Sendable {
    ...
    private func put(_ documentID: String, _ request: HTTPRequest) -> HTTPResponse {
        ...
        if let existing = docs[documentID] {
            ...
            // revision at all brings the document back. This mock used to accept the revisioned
            // form, which is why an engine that retried its resurrection at `remote.rev` passed
            // every test here and then span forever against a real server.
            if existing.deleted {
                if providedRev != nil { return conflict() }
            } else {
