...
/// One folder of the sidebar's tree, resolved out of the store ahead of rendering so row bodies
/// never have to walk it.
///
/// Folders only: the sidebar answers *where am I*, and the notebooks filed in a folder are the
/// answer to *what is here* — which is the grid's question, and the grid can say it with covers,
/// page counts and dates. Listing every note in both columns doubled the library on screen and
/// made the one column that should stay short as long as the other.
struct LibraryNode: Identifiable, Hashable {
    /// The folder id. Also the row identity, now that the tree holds nothing else.
    let id: String
    let title: String
    /// Direct children — subfolders *and* notebooks, which is what the row's number counts even
    /// though only the subfolders are drawn beneath it.
    let count: Int
    let provenance: SyncProvenance
    /// `nil` (not `[]`) for a folder with no subfolders — that is what suppresses the
    /// disclosure triangle.
    let children: [LibraryNode]?

    /// Builds the root-level folders, each holding its subfolders. A `parentFolderId` cycle
    /// (possible in a hand-edited or half-merged folders.json) is broken by tracking the ids
    /// already on the path.
    @MainActor
    ...
    /// collapsed, and the point of this sidebar is that the folders are already on screen.
    /// `collapsed` therefore holds what the user has explicitly shut, so a folder that arrives
    /// from a sync shows its subfolders without being opened first.
...
/// Left column of the library: the root item, then every folder, nested. Each row carries the
/// number of things filed directly in it and a provenance glyph (on server / local only), and can
/// be renamed, moved or thrown away from its own row.
struct LibrarySidebar: View {
    ...
    private func folderRow(_ row: LibraryRow) -> some View {
        let node = row.node
        let selected = selection == .folder(node.id)
        return HStack(spacing: 4) {
            disclosure(for: node)
            Button {
                selectDestination(.folder(node.id))
            } label: {
                rowBody(
        ...
        .padding(.trailing, 8)
        .background(selected ? Modernist.ink : .clear)
        .accessibilityIdentifier("sidebar.folder.\(node.id)")
        .accessibilityAddTraits(selected ? .isSelected : [])
    ...
    private func nodeOptions(_ node: LibraryNode, selected: Bool = false) -> some View {
        ...
        .accessibilityIdentifier("sidebar.options.\(node.id)")
    }

    @ViewBuilder
    private func nodeMenu(_ node: LibraryNode) -> some View {
        Button {
            beginRename(node.id, current: node.title)
        } label: { Label("Rename", systemImage: "pencil") }
        Menu {
            Button("Library") { move(node, to: nil) }
            let excluded = Set(store.deletionScope(ofFolder: node.id).folderIDs)
    ...
    private func move(_ node: LibraryNode, to folderId: String?) {
        perform("Moving \(node.title)", error: $actionError) {
            try store.moveFolder(id: node.id, toFolder: folderId)
        }
    }

    private var trashMessage: String {
        guard let node = trashTarget else { return "" }
        let scope = store.deletionScope(ofFolder: node.id)
    ...
    private func trashSelection() {
        ...
        perform("Moving \(node.title) to the Trash", error: $actionError) {
            let scope = Set(store.deletionScope(ofFolder: node.id).folderIDs)
            try store.trashFolder(id: node.id)
    ...
    /// Shared row interior, so a folder and the root item line up on the same grid.
    ...
    private func beginRename(_ folderId: String, current: String) {
    ...
    private func commitRename() {
        ...
        guard !title.isEmpty, let renamingFolderId else { return }
        perform("Renaming", error: $actionError) {
            try store.renameFolder(id: renamingFolderId, title: title)
        }
    }
